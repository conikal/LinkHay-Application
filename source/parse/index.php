...
if($url == null || $url == ""){
	echo json_encode(array("isSuccess" => false, "errMsg" => "URL_NOT_AVAILABLE"));
	return;
...
if($content == "" || $content == null){
	echo json_encode(array("isSuccess" => false, "errMsg" => "CONTENT_NOT_AVAILABLE"));
	return;
}
...
// print_r($metatags);
// print_r($sitedata);
//echo json_encode(array("isSuccess" => true, "url" => $url, "data" => $dataArr));
// print_r($relatedImages);
...
echo json_encode(array("isSuccess" => true, "url" => $url, "data" => $sitedata, "images" => $relatedImages));		
ob_flush();
flush();
}catch(Exception $e){
	echo $e->getMessage();
